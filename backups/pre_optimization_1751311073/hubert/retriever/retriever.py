...
class HybridRetriever:
    ...
    def retrieve(self, query: str, filters: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        # Optimized retrieval limits - reduced over-fetching
        if self.use_hybrid_search and self.use_reranker:
            retrieval_limit = min(self.top_k * 2, 50)  # Cap at 50 to prevent over-fetching
        elif self.use_hybrid_search:
            retrieval_limit = min(self.top_k * 1.5, 30)  # Reduced from 3x to 1.5x
        elif self.use_reranker:
            retrieval_limit = min(self.top_k * 1.5, 25)  # Reduced from 2x to 1.5x
        else:
        ...
        # Early termination - only rerank if we have enough good results
        if self.use_reranker and self.reranker and len(final_results) > 0:
            # Only rerank top candidates to save time
            rerank_limit = min(len(final_results), self.top_k * 2)
            rerank_candidates = final_results[:rerank_limit]
            
            pairs = [[query, res['content']] for res in rerank_candidates]
            if pairs:
                scores = self.reranker.predict(pairs)
                for i, res in enumerate(rerank_candidates):
                    res['reranked_score'] = scores[i]
                rerank_candidates.sort(key=lambda x: x.get('reranked_score', 0), reverse=True)
                
                # Combine reranked results with remaining results
                final_results = rerank_candidates + final_results[rerank_limit:]
