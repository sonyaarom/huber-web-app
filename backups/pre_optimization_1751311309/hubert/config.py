...
class Settings(BaseSettings):
    ...
    # --------------------------------------------------------------------------
    max_retrieval_limit: int = Field(env='MAX_RETRIEVAL_LIMIT', default=50)
    enable_similarity_threshold: bool = Field(env='ENABLE_SIMILARITY_THRESHOLD', default=True)
    similarity_threshold: float = Field(env='SIMILARITY_THRESHOLD', default=0.3)
    hnsw_ef_search_multiplier: int = Field(env='HNSW_EF_SEARCH_MULTIPLIER', default=2)
    db_pool_min_conn: int = Field(env='DB_POOL_MIN_CONN', default=5)
    db_pool_max_conn: int = Field(env='DB_POOL_MAX_CONN', default=25)
    ...
    reranker_max_candidates: int = Field(env='RERANKER_MAX_CANDIDATES', default=20)
