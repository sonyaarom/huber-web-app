...
class PostgresStorage(BaseStorage):
    ...
    def __init__(self, db_params: Dict[str, Any] = None, min_conn: int = 5, max_conn: int = 25):
    ...
    def vector_search(self, table_name: str, query_embedding: List[float], limit: int = 5, filters: Dict[str, Any] = None, threshold: float = None) -> List[Dict[str, Any]]:
        """Perform a vector search with optimized HNSW parameters."""
        params = [np.array(query_embedding)]
        
        # Optimize ef_search for query performance
        ef_search = max(limit * 2, 100)  # Dynamic ef_search based on limit
        ...
        if threshold:
            where_clauses.append("(1 - (t.embedding <-> %s)) >= %s")
