...
#     """
#     logger.info("Starting sitemap processing utility")
    
#     try:
#         url = settings.url
#         logger.info(f"Using sitemap URL: {url}")
...
#         matches_dict = process_sitemap(
#             url, pattern, exclude_extensions, exclude_patterns, 
#             include_patterns, allowed_base_url
#         )
        
#         # Display results
#         logger.info(f"Final URL count: {len(matches_dict)}")
#         logger.info(f"First 10 entries: {list(matches_dict.values())[:10]}")
#         return matches_dict
...
#     result = main()
#     if result is None:
#         logger.error("Script failed with errors")
#         sys.exit(1)
